...
export async function danisanRoutes(app: FastifyInstance) {
  ...
  app.put("/api/danisan/me", { preHandler: requireRole("danisan") }, async (request, reply) => {
    ...
    if (!existing) {
      const [created] = await db.insert(danisan).values({ ...body, userId: sub }).returning();
      if (!created) return reply.status(500).send({ success: false, error: "Profil olusturulamadi" });
      await createAuditLog({ userId: sub, action: "create", tableName: "danisan", recordId: created.id, request });
      return reply.status(201).send({ success: true, data: created });
    }

    const [updated] = await db.update(danisan).set({ ...body, updatedAt: new Date() }).where(eq(danisan.userId, sub)).returning();
    await createAuditLog({ userId: sub, action: "update", tableName: "danisan", recordId: existing.id, request });
    return reply.send({ success: true, data: updated });
  });

  // GET /api/danisan/list - Egitmen/admin icin danisan listesi
  app.get("/api/danisan/list", { preHandler: requireRole("egitmen", "admin") }, async (request, reply) => {
    ...
    if (query.search) {
      ...
      results = await db
        .select({
          id: danisan.id, userId: danisan.userId,
          firstName: users.firstName, lastName: users.lastName,
          phone: users.phone, city: danisan.city,
          gender: danisan.gender, bloodType: danisan.bloodType,
          chronicDiseases: danisan.chronicDiseases,
          mainComplaints: danisan.mainComplaints, createdAt: danisan.createdAt,
        })
        .from(danisan)
        .innerJoin(users, eq(danisan.userId, users.id))
        .where(or(ilike(users.firstName, term), ilike(users.lastName, term), ilike(danisan.city, term)));
    } else {
      results = await db
        .select({
          id: danisan.id, userId: danisan.userId,
  ...
  app.get("/api/danisan/:userId/full", { preHandler: requireRole("egitmen", "admin") }, async (request, reply) => {
    ...
    // Tedavi gecmisi
    const tedaviler = await db.select().from(tedavi).where(eq(tedavi.danisanId, userId)).orderBy(desc(tedavi.treatmentDate)).limit(20);

    // Tahlil sonuclari
    const tahliller = await db.select().from(tahlil).where(eq(tahlil.danisanId, userId)).orderBy(desc(tahlil.testDate)).limit(20);

    // Randevular
    const randevular = await db.select().from(randevu).where(eq(randevu.danisanId, userId)).orderBy(desc(randevu.scheduledAt)).limit(20);
  ...
  app.get("/api/egitmen/search", { preHandler: requireAuth() }, async (request, reply) => {
    ...
    const all = await db
      .select({
        id: egitmen.id, userId: egitmen.userId,
        firstName: users.firstName, lastName: users.lastName,
        specialties: egitmen.specialties,
        clinicName: egitmen.clinicName, clinicCity: egitmen.clinicCity,
    ...
    let results = all;
    if (query.city) {
      results = results.filter((e) => e.clinicCity?.toLowerCase().includes(query.city!.toLowerCase()));
    }
    if (query.specialty) {
