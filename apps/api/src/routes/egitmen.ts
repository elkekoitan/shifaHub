...
export async function egitmenRoutes(app: FastifyInstance) {
  // ─── GET /api/egitmen/list — Danisanlar icin egitmen arama ───────────────
  app.get("/api/egitmen/list", { preHandler: requireAuth() }, async (_request, reply) => {
    const egitmenler = await db
      .select({
    ...
    return reply.send({ success: true, data: egitmenler });
  });
  // GET /api/egitmen/me
    ...
    async (request, reply) => {
      ...
      for (const danisanUserId of allIds) {
        const [user] = await db
          ...
          .from(users)
          .where(eq(users.id, danisanUserId))
          .limit(1);
        const [profil] = await db
          ...
          .from(danisan)
          .where(eq(danisan.userId, danisanUserId))
          .limit(1);
