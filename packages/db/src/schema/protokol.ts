import { pgTable, uuid, varchar, text, jsonb, pgEnum, index } from "drizzle-orm/pg-core";
...
export const protokol = pgTable(
  ...
  (t) => [
    index("protokol_danisan_id_idx").on(t.danisanId),
    index("protokol_egitmen_id_idx").on(t.egitmenId),
  ],
);

export type Protokol = typeof protokol.$inferSelect;
export type NewProtokol = typeof protokol.$inferInsert;
