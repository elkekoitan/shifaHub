...
export const stokHareket = pgTable(
  "stok_hareket",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    stokId: uuid("stok_id")
  ...
  (t) => [
    index("stok_hareket_stok_idx").on(t.stokId),
    index("stok_hareket_user_idx").on(t.userId),
    index("stok_hareket_tedavi_idx").on(t.tedaviId),
  ],
);

export type Stok = typeof stok.$inferSelect;
export type NewStok = typeof stok.$inferInsert;
export type StokHareket = typeof stokHareket.$inferSelect;
export type NewStokHareket = typeof stokHareket.$inferInsert;
